...
# (incl. the hub esb-toxiproxy), both in-container AND from the host
# (published 127.0.0.1 forwards).
# The host-forward probes exist because rootlesskit forwards are the one
# failure mode this host has produced repeatedly (seen live on this host: dead 8474,
...
# 6. esb-toxiproxy (the hub) - host-network container: the API port IS a host port.
# ALL hub fault injection binds through it, and a silent death here is exactly
