# tests/etl-int — Phase-3 ETL data-plane integration suite

BUILD-QA suite for the SILKROUTE ETL data plane (apps/modern-oms, apps/cdc,
...
committed; ORCH-LEAD copies what it needs into `evidence/`).
...
1. **`1-clean-slate-pipeline`** — from `make etl-reset`, boots
   ...
   *Proves:* the seeded-order-day acceptance — gold tables with 100%
   reconciliation (MASTER_PROMPT §6).
...
- On success the suite ends with `make etl-reset` so ORCH-LEAD and the
  critic always start from a known clean slate.
...
- No Makefile target is wired for this suite yet: `make etl-int` (or
  similar) is requested from ORCH-LEAD; until then run the script directly.
