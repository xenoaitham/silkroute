# SILKROUTE operations - Chaos runbook: ESB order-mediation steady-state harness

Companion of `tests/chaos/chaos-run.sh` (the executable) and `tests/chaos/esb-faults.sh`
(the stable integration-hub toxiproxy wrapper - called, never modified). Company context:
...
proven point-wise in integration hub (evidence E-009). This harness does something those
...
- **No freshness scope.** C4 (CDC freshness / T+1 batch) has no operations surface
...
derived from. The OBSERVED sections in only get filled after executions; if a
...
backoff 200 ms x 2.0 (INFRA failures only, never business faults); resilience4j
...
- **Severe variant (operator choice, 2500 ms injection):** at 2500 ms the
 ...
 (2 s x 3 attempts + 200/400 ms backoffs), the breaker (50 % over >= 6 calls in a
...
- **During the failure window (ERP process killed):** probes show explicit fast
 ...
 ~2 s x 3 + 600 ms of backoff, and the open breaker turns later calls into
...
- **No hangs - the falsifiable edge:** if probes HANG (the ESB's Redis client
 ...
 bounded"), recorded in ; it is not a harness failure.
...
- Pre-fix (default Spring task executor): H1 FAILED - k6 p95 pinned at its 30 s
 ...
 slowed sagas (~3.2 s = 4 proxied calls x 800 ms; solo probe 3.22 s vs 15.6 ms
...
- Post-fix (canonical, E-022): in-window probes n=25, med 3.6 s, max 4.1 s -
 ...
 latency inflation (15 ms -> ~3.6 s under 800 ms x 4 calls) is the honest shape.
...
- Pre-fix: H3 FAILED - every outage probe blocked past the 10 s cap; k6 39.65 %
 ...
 window. Post-heal 19x 500 s = the queued backlog dying at the async timeout.
