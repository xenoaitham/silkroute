...
| E-006 | legacy-erp runtime: seeds exactly 50 SKUs / 8 stores (CA×3, SG×3, CN×2) / 400 stock rows, demo generator produces orders through the service layer, and ORCH-LEAD's independent SOAP 1.2 suite (happy + fault + auth + ?wsdl) passes 16/16 | apps/legacy-erp/, scripts/wss-header.sh | `SERVER_PORT=18080 ERP_DEMO_GENERATE_ORDERS=5 java -jar apps/legacy-erp/target/legacy-erp-*.jar` then `bash /tmp transcript in artifact` (suite script inline in artifact) | 2026-09-09: `ERP SEED: skus=50 stores=8 stockRows=400`; `ERP DEMO: requested=5 submitted=5 skipped=0`; verify PASS=16 FAIL=0 (SGD 490 minor, CAD order totals, SG region in getStock, OutOfStockFault carries availableQuantity, no over-allocation); app stopped + 18080 released after capture. See evidence/runs/E-006-erp-runtime-verification.txt |
