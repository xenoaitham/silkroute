...
> **Self-directed reference implementation (2026).** The scenario company **Maple Retail Group** is **fictional** - no real client, employer, or engagement is implied ([docs/scenario-charter.md](docs/scenario-charter.md)). The project lives under *Projects*, never *Experience*.

<img src="demo/screenshots/hero.png" alt="SilkRoute - the bronze lake, region-partitioned" width="100%">
...
**Status: fully built and measured end to end** - order mediation through the legacy estate, change data capture, and the governed lake all run in sim with evidence (E-030..E-034 for the data plane). **36 evidence rows** in the ledger ([evidence/EVIDENCE.md](evidence/EVIDENCE.md), E-001..E-036), each with a re-runnable command. CI is green 5/5 (run 34789287783, sha d3ec023, E-036).
...
The data plane - the OMS event store, Debezium CDC capture to bronze, and the Spark silver/gold batch with DQ gates + reconciliation - is designed in [docs/lld.md](docs/lld.md) and [ADR-0006](decisions/ADR-0006-etl-data-plane.md).
...
scripts/demo-order.sh # -> HTTP 201: orderId ORD-2026-00000N, saga order/reserve/pricing/confirm all
 ...
 # totalAmount {"amountMinor":1000,"currency":"CAD"} (2 x 500 minor, SKU-0001 @ 500 CAD)
bash tests/chaos/esb-faults.sh latency 800
scripts/demo-order.sh demo-lat # -> HTTP 201 again but ~3.3 s slower (4 proxied ERP calls x 800 ms;
...
**Further reading:** [docs/slo-report.md](docs/slo-report.md) (measured SLOs), [docs/cost-model.md](docs/cost-model.md) (priced cost sheet), [tests/chaos/README.md](tests/chaos/README.md) (chaos runbook), [docs/hld.md](docs/hld.md), [docs/lld.md](docs/lld.md), [docs/delivery-model.md](docs/delivery-model.md), [docs/runbooks.md](docs/runbooks.md), [demo/](demo/) (screenshot walkthrough).
