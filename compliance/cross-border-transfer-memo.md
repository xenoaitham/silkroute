...
|---|---|---|---|
| A1 | Canada HQ ↔ Singapore hub (orders, inventory, pricing, events for **non-China** customers) | both | Planned flow for non-restricted data; instrument required before go-live  |
| A2 | China partition ↔ anywhere | none exists | **Design position: no transfer.** The CN partition has no egress path; the Art. 38 analysis in exists only for the day someone proposes to change that |
