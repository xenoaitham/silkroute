# SILKROUTE ESB (apps/esb) - integration hub integration hub
...
minor units, C5). Downstream consumers (the data-plane OMS event store) rebuild
...
| `PII_MASK_SECRET` | `sim-egress-secret` | keyed-HMAC secret for CN egress pseudonymization (C1); move to a real secret store in compliance |
