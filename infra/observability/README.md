...
- **Log stores**: `esb-app` (SAE application logs, 30-day TTL),
 ...
 compliance), `orders-events` (order event traces, 30-day TTL),
 `pipeline-metrics` (30-day TTL - the DESIGNED home for the data-plane CDC/batch
...
- **Store indexes** (`alicloud_log_store_index` x 4): full-text on every
...
- **SLS alerts (modern `alicloud_sls_alert` form, migrated 2026-09-13 from the
 ...
 to `topic: silkroute-esb-dlq`, fires reviewal (severity 8) when the count
 ...
 `compliance/audit-trail-design.md` . Who/where refinement is the analyst
 cookbook ( Q2).
...
1. ~~`notification_list` deprecation~~ **RESOLVED 2026-09-13 :** all SLS
