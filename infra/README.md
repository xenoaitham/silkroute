# SilkRoute landing zone - AliCloud (landing zone, validated-plans mode)
...
with placeholder credentials, but nothing is ever applied. Every landing zone
...
| `observability/` | Log project `silkroute-sg` (stores: `esb-app` 30d, `audit` 180d, `orders-events` 30d, `pipeline-metrics` 30d - the freshness contract home; its producer is live in sim, E-030/E-031, and SLS ingest verifies at activation), 4 store indexes, 4 `alicloud_sls_alert` rules (DLQ depth, C3 p95-budget breach, denied-action burst, trail-tamper tripwire), ActionTrail -> audit store, overview dashboard (freshness panel titled for the live `cdc_freshness_seconds` metric; it renders in the cloud only at activation), CMS contact group + SAE-CPU / RDS-connection alarms (details: `observability/README.md`, E-019) |
...
- **C7 (PIPEDA cross-border):** the SG hub is the documented transfer point for
 ...
 (transfer-mechanism memo lives in `docs/`, compliance wiring).
...
3. `make deploy-sg` (guarded), capture plan/apply, then `make destroy` - .
