...
- AliCloud Kafka `silkroute-cn-kafka` + topics `silkroute-orders-events` /
  `silkroute-esb-dlq` (dot-free ApsaraMQ names — the service rejects dots — selected
  via env indirection; separate brokers, no replication to SG)
...
```bash
terraform plan -var enable_cn_region=true   # "Plan: 106 to add, ..." - evidence only
```
