...
- - never leave billable resources running; no load balancers/nodepools overnight.
...
- Cloud mode = Terraform modules under `infra/` (alicloud provider) + the same apps deployed to SAE/ACK, used only for landing zone/6 evidence capture, then `make destroy`.
...
- (−) OSS access in cloud mode via the S3-compat layer has known gaps vs the native OSS SDK (region endpoints, STS token flow, SSE-KMS header semantics); native ossutil/oss2 paths to be validated at landing zone before any cloud evidence relies on them.
