...
1. **SAE (`alicloud_sae_application`)** - pay-per-use (replica-hours), deploys the existing fat jar from the artifacts bucket, namespaces give prod/CN isolation, built-in autoscaling rules, no cluster to run. Matches .
2. **ACK managed nodepool with spot instances** - deeper raw-Kubernetes surface, but an always-on nodepool costs money while idle ( violation risk), adds cluster ops burden no scenario constraint requires.
...
- (−) Raw Kubernetes depth (PDBs, node pools, spot interrupts) is not demonstrated by this phase - acknowledged; ACK exposure can be a operations/7 amplifier if time allows.
- (−) The SLS alert on DLQ depth uses the deprecated `notification_list` argument. CORRECTED per review cycle 2: the modern `alicloud_sls_alert` resource DOES exist in provider 1.285.0 (schema-verified); migrating to it is a small follow-up, deferred - trade-off recorded in `infra/observability/README.md`.
 **Status update (2026-09-13, operations/S7):** the migration landed - `alicloud_sls_alert.dlq_depth` replaced the legacy form, joined by three new alerts (C3 p95 budget breach, denied-action burst, trail-tamper tripwire). Landing-zone counts moved 79->87 (SG) / 110->118 (CN flag on); one vocabulary divergence (`group_configuration.type` takes `no_group`, not the legacy `no_condition`) was caught live by the plan validator during the migration. Evidence: E-019.
