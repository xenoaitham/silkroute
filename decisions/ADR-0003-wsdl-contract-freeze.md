...
- **Status:** Accepted (freeze effective 2026-09-09, commit following ERP integration contract-test PASS)
- **Deciders:** Haitham
...
- shared `common-v1` (MoneyType = integer minor units + CAD/SGD/CNY enum per C5; StoreRegionCodeType CA/SG/CN - load-bearing for integration hub content-based routing; LegacyAuditType; ErpFaultType base)
...
1. **No formal freeze** (keep editing WSDLs when convenient): destroys the C6 discipline that integration hub's impedance-mismatch work (XSLT, canonical model) is supposed to demonstrate. Rejected.
...
- (+) integration hub can now freely demonstrate XSLT/canonical-model impedance handling against a contract that provably cannot move - the entire point of C6.
- (+) The freeze is auditable: ADR + tag + git history + a CI-enforced tripwire.
- (+) Money/region discipline (C5, integration hub routing input) is schema-enforced, not just documented.
- (−) Any integration hub+ discovery that the contract is *inconvenient* must be absorbed by the ESB layer (that is the exercise) or handled by versioning, never by editing v1.
- (−) **Business faults surface as `soap:Receiver`, not `soap:Sender`** - CXF's default code for declared faults (observable on the wire: an invalid SKU returns `<soap:Value>soap:Receiver</soap:Value>`). Semantically these are Sender-class errors (the caller sent the bad request), but the frozen WSDLs do not constrain fault codes and editing the implementation's fault-code mapping is not worth the churn for a sim. Recorded here so it is design, not surprise: **the integration hub ESB must map `ErpFaultType` detail elements -> Sender-class semantics when translating SOAP faults to canonical errors**, and that mapping is a integration hub acceptance item.
