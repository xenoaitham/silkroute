...
> **Explicit label:** this document describes the **DESIGNED** operating model for the fictional Maple Retail Group rollout - how the platform **would** be staffed and run. Nothing here is a claim about real operations: no cloud account exists (ADR-0002), the cloud side is validated IaC that has never been applied, and Maple Retail Group is fictional (self-directed reference implementation, 2026; framing). Verbs are deliberately conditional ("would", "is designed to").
...
3. Project state and post-mortem notes are written down at the end of every work session; the next session starts from the recorded state, not from memory.
...
- **Capture-then-destroy discipline.** Per the cost guardrails, cloud mode exists for evidence capture only: deploy -> capture -> `make destroy`; billable resources are never left running . The activation runbook in [docs/runbooks.md](runbooks.md) operationalizes this.
