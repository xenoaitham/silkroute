...
| ETL — BUILT, sim-measured (Phase 3) | Q21–Q24 (CDC, lake layering, DQ gates, reconciliation + windows) |
...
## ETL — built and sim-measured (Phase 3, ADR-0006)

### Q21. How does the CDC pipeline work?

Straight answer first: it's built and measured in sim — I'll quote the numbers and the mode. The flow: 30 REAL orders driven through the live ESB (REST façade → SOAP saga → ERP; no synthetic SQL) publish success events to Kafka; the OMS event store consumes them into MySQL `silkroute_oms` with an at-least-once replay guard; an embedded Debezium engine captures the MySQL binlog into Kafka `silkroute.cdc.oms`; a bronze writer lands write-once, region-tagged JSONL objects on the S3 API. Binlog capture is proven, not claimed: 60 `CDC-CAPTURE op=c ... sourceTsMs=...` lines in the seeded-day run — per-record source-commit timestamps that can only come from the binlog stream, because the engine has no poll fallback. Freshness is the same measure-don't-assert discipline as the latency budget: the producer itself emits `cdc_freshness_seconds` per published batch and on an idle heartbeat — value 0 at landing, growing under source silence, 0 through 213 seconds across session runs, never near the 900-second C4 budget. One honest scope line: the embedded engine is the SIM deployment shape — cloud parity is at the Debezium-envelope-and-topic interface (a Connect cluster feeds the same bronze writer), not at the runtime, and SLS ingest of the metric verifies at activation.

Evidence/anchor: E-030 (pipeline + recon + batch window), E-031 (freshness metric + binlog proof); ADR-0006 decision 1; `docs/infra-env-parity.md`.

### Q22. What does the lake layering look like?

Built in sim, exactly as designed. Bronze is raw and immutable: Debezium envelopes landed as-is as JSONL keyed `region=<R>/table=<t>/dt=<UTC date>/...` — 50 objects in the measured run — no in-place edits, so it doubles as the replay source and the audit trail. Silver is cleansed and conformed: latest-per-PK dedup, typed columns, money as BIGINT minor units plus currency code (C5 — no float types anywhere in the module), region preserved on every row. Gold is the star schema: `fact_orders`, `fact_order_lines`, `dim_sku`, `dim_store`, `dim_date` as Parquet. The gold grain decision matters for reconciliation — one row per order line, so totals recompute bottom-up against the source, which is exactly what the recon then does. What's still environment-honest: these buckets are MinIO in sim standing in for the OSS family `silkroute-sg-{bronze,silver,gold}` — same names on purpose; the OSS S3-compatible layer differs in parts, so the native OSS SDK path verifies at activation.

Evidence/anchor: E-030 (50 bronze objects, gold built, recon all-match); E-033 (write-once, region-tagged, zero delete/copy code paths); ADR-0006 decision 3; `apps/batch/README.md`.

### Q23. Where do the data-quality gates sit?

Built, and falsifiable — that's the part I'd stress. Three rule families run between silver and gold: completeness, uniqueness, and referential — implemented as pure-Java predicates (unit-testable without Spark), and a `selftest` runs fixtures with KNOWN violations through the same predicates the pipeline uses: all 6 rule-table checks catch their own fixture. Violations don't pollute gold or fail silently — they route to a MySQL quarantine table with the rule that failed, the offending payload, and the run ID, and a non-empty quarantine turns the run red by design. The reconciliation gate can fail too: a phantom source row injected into the OMS store flips `recon-only` to exit 2 with allMatch=false, and removing it restores green — 100% reconciliation is an assertion, not a construction. The residency gate carries over from the ESB layer: every CN customer_ref in bronze is the ESB-masked `msk-*` value — zero plaintext occurrences across all bronze objects — and the writer rejects any record whose region is missing or outside the allowlist, so an untagged object can never land.

Evidence/anchor: E-032 (DQ selftest + recon negative control), E-033 (bronze immutability + C1 mask accounting), E-034 (the integration suite re-proves both); E-009/E-016 (masking proven at the ESB egress, chokepoint checked in CI).

### Q24. How do you prove correctness — reconciliation and the T+1 window?

Measured on a seeded 30-order day. Reconciliation: after the batch, source counts and totals are recomputed from the OMS MySQL against the gold Parquet re-read from the bucket — orders 30=30, lines 30=30, and per-currency sums all matching: CAD 80 849, CNY 173 205, SGD 40 605 minor, `allMatch=true`. Currencies are never summed across — per-currency by construction (C5). The T+1 window is evaluated in code by a unit-testable evaluator: `ZoneId Asia/Singapore` pinned, window end 06:00, an injectable clock whose negative test proves a run past 06:00 reads `withinWindow=false`. The measured run completed at 04:52:09 Singapore time — `withinWindow=true`, a real in-window completion — with `batch_completion` at 41 seconds wall. Honest boundary, and I say it before anyone asks: the sim job runs on demand, so the window evaluator proves the logic and the run proves a real in-window completion — sustained production lag in a continuously-running deployment is what a real T+1 schedule would additionally prove, and SLS ingest of the metric verifies at activation.

Evidence/anchor: E-030 (recon all-match, batch window), E-031 (freshness contract), E-032 (recon can fail); ADR-0006 decision 4; C4/C5 in `docs/scenario-charter.md`.
...
Design-level, and I won't dress it up as delivery experience. Environment promotion: dev/staging/prod as separate Terraform root compositions with separate state backends and per-env variables — the CN partition stays flag-off everywhere except a deliberately created CN workspace, because "CN off" must be the default. Decision discipline: the six ADRs in this repo are the template — every consequential choice (dual-mode, account path, contract freeze, compute, region strategy, the ETL data plane) gets context, options, decision, consequences, and a supersede path. Runbooks for the operations that matter: activation, deploy/destroy, DLQ redrive, freshness breach. Cadence: the repo's own rhythm generalizes — small slices, each ending with green CI and updated evidence, decisions logged before code. Residency audits on a schedule: tag greps plus the CI residency tests in the environment, reported alongside the reconciliation and freshness metrics.

Evidence/anchor: `decisions/` (6 ADRs); `infra/README.md` (env/backend examples, activation checklist); MASTER_PROMPT §6 Phase 7 (delivery model intent). No team-size or delivery-experience claims.
