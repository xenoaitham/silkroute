...
- Designed multi-region IaC (Terraform 1.16, alicloud provider 1.285.0) with a Singapore hub (79 planned resources) and a China data-residency partition (110 with the CN flag on) — residency enforced at the provider graph via an aliased `alicloud.cn` provider, with least-privilege RAM, KMS/SSE-KMS on OSS and RDS, SAE compute, and no cross-region replication anywhere (E-012).
