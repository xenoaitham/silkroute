...
7. **Deploy -> capture -> destroy** per the guardrails (next runbook): `make deploy-sg` -> capture plan/apply output + runtime verifications from the checklist step 5 of infra/README.md (SSE-KMS upload with `kms:GenerateDataKey`, SLS ingest, STS assume-role, TDE on the SG RDS instance - the CN instance belongs to the separate CN-account activation) -> `make destroy`. New evidence rows say `cloud` mode; existing rows keep their sim numbers.
...
**Trigger:** evidence-capture windows only (per : never leave billable resources running overnight).
