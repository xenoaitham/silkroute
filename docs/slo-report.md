...
Maple Retail Group is a **fictional** company; this is a design artifact of a **self-directed reference implementation**. Framing per : self-directed project, never presented as professional delivery experience.
...
- **No number in this report is asserted without an evidence row.** The "Measured " column names its row and artifact; rows live in `evidence/EVIDENCE.md` with 1:1 reproduce commands.
...
| **Measured ** | Pending - filled from the S7 sustained run (E-020) below. |

**Measured :** **E-020 (canonical, 2026-09-13):** p95(all) **131.6 ms** / **p99(all) 236.2 ms** @ 19.49 RPS achieved (20 target), 3 704 iterations / 190 s wall (10 s ramp + 180 s hold), 0 network failures - the 352 non-201s verified 1:1 as 422 INV-OUT-OF-STOCK stock-exhaustion business rejections (log-grep = 352 exactly; E-010 convention). The sustained profile tracks 201-only correctness via the exact 1:1 422 accounting (tag-split percentiles are the stress profile's instrument: its 201-only r20 step measured p95 20.4 ms). Budget consumption: **44% at p95, 79% at p99**. Honest variance note: an identical-shape 20 RPS window earlier the same session measured p95 31.8 ms - a 4x swing from shared-host contention alone (E-010 precedent for the hardware note). Sim mode, k6 local (not CI). -> evidence **E-020**.
